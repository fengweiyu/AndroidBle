...
public class BleDevice {
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
		...
		try {
			...
			LogUtil.i(BleDevice.this, "device " + getDeviceName()
                        + " disconnected !close ignoring: "+e);
		}	
               // close();
            }		
        }
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt,
            ...
            doCharacteristicChanged(characteristic.getUuid(),characteristic.getValue());
        }
                ...
                synchronized (mBTGattWriteCharacteristicListLock) {
                    for (int i = 0; i < mBTGattWriteCharacteristicList.size(); i++) {
                        if (writeDataBuf.length - writeDataBufIndex <= WRITE_CH_SIZE_MAX) {
                            subValue = new byte[writeDataBuf.length-writeDataBufIndex];
			    System.arraycopy(writeDataBuf,writeDataBufIndex,subValue,0,writeDataBuf.length-writeDataBufIndex);			
                            writeDataBufIndex = writeDataBuf.length;
                        } else {
                            subValue = new byte[WRITE_CH_SIZE_MAX];
			    System.arraycopy(writeDataBuf,writeDataBufIndex,subValue,0,WRITE_CH_SIZE_MAX);;
    ...
    private byte[] writeDataBuf = null;
    ...
    public boolean write(byte[] value) {
        if (mBluetoothGatt == null || mBTGattServiceList == null)
            return false;
        synchronized (writeDataBufLock) {
            if ((writeDataBuf != null)||(value==null)) {
                LogUtil.e(BleDevice.this,
            ...
            writeDataBuf = Arrays.copyOf(value, value.length);
            writeDataBufIndex = 0;
            if (writeDataBuf == null)
                return false;
            mCommandHandler.post(new MySendCmdThread());
     ...
     * @param data 数据
     */
    private void doCharacteristicChanged(UUID uuid, byte[] data) {
        if (mCallback != null) {
