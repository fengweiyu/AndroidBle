...
class HandProtocolInfo
{
	...
	{
		public static final byte OWN_NUMBER					=(byte)0x01;		//本机号码//
		public static final byte PASSWORD_SET				=(byte)0x02;		//????????//
		public static final byte CENTRE_IP_PORT				=(byte)0x03;		//????IP????????//
		public static final byte APN_NAME						=(byte)0x04;		//APN//
		public static final byte WORKMODE					=(byte)0x05;		//??????//
		public static final byte CONTROL_NUM0				=(byte)0x06;		//???????????0//
		public static final byte SMS_CENTER_NUM0			=(byte)0x07;		//???????????0//
		public static final byte LISTEN_NUM0 					=(byte)0x08;		//???????????0//
		public static final byte  FIXED_NUM					=(byte)0x09;		//????????绰????//
		public static final byte  MAX_SPEED					=(byte)0x0a;		//???????//
		public static final byte  LOW_VOLTAGE 				=(byte)0x0b;		//????????//
		public static final byte  ALARM_INTERVAL				=(byte)0x0c;		//????????????//
		public static final byte  GPRS_TIMEOUT				=(byte)0x0d;		//GPRS??????????//
		
		public static final byte  GPI_STATE						=(byte)0x0f;		//???????//
		public static final byte  SPEAKING_MODE				=(byte)0x10;		//?????//
		public static final byte  SAVEING_POWER_MODE		=(byte)0x11;		//?????//
		public static final byte  CONTROL_NUM1				=(byte)0x16;		//???????????1//
		public static final byte  SMS_CENTER_NUM1			=(byte)0x17;		//???????????1//
		public static final byte  LISTEN_NUM1 					=(byte)0x18;		//???????????1//
		...
		public static final byte  TRANSFER_INTERVAL			=(byte)0x4e;		//????OBD?????????????//
		public static final byte  OEMID						=(byte)0x50; 	 	// ?豸???????
		public static final byte  LICENSE_PLATE				=(byte)0x46;		// ???????
		public static final byte  LICENSE_PLATE_COLOR 		=(byte)0x52;  		// ???????
 
	}
	
	class HandProtocolFunSubCmdId
	{
		public static final byte  FACTORY_SET					=(byte)0x0a;//恢复出厂设置	//
	}
